...
def build_options(sprite_names=None, msu_names=None):
    ...
    opts = [
        ...
        ("BOW_MODE", "Bow Mode",
         ["progressive", "silvers", "retro", "retro_silvers"], 0),
        ...
        ("ALGORITHM", "Item Placement",
         ["balanced", "vanilla_fill", "major_only", "dungeon_only",
          "district"], 0),
        ("PROGRESSIVE", "Progressive Items", ["on", "off", "random"], 0),
        ("ACCESSIBILITY", "Accessibility", ["items", "locations", "none"], 0),
        ("HINTS", "Hints", ["on", "off"], 0),
        ("SHUFFLE_FOLLOWERS", "Shuffle Followers", ["off", "on"], 0),
        ("SHOPSANITY", "Shopsanity", ["off", "on"], 0),
        ...
        ("DOOR_TYPE_MODE", "Door Type",
         ["original", "big", "all", "chaos"], 0),
        ("TRAP_DOOR_MODE", "Trap Doors",
         ["vanilla", "optional", "boss", "oneway"], 0),
        ("KEY_LOGIC_ALGORITHM", "Key Logic",
        ...
        ("OW_CROSSED", "Crossed Overworld",
         ["none", "grouped", "polar", "unrestricted"], 0),
        ("OW_FLUTESHUFFLE", "Flute Spots",
         ["vanilla", "balanced", "random"], 0),
        ("OW_UNPARALLEL", "Unparallel Worlds", ["off", "on"], 0),
        ("OW_TERRAIN", "Terrain Shuffle", ["off", "on"], 0),
        ...
        ("COMPASSSHUFFLE", "Compass Shuffle",
         ["none", "nearby", "wild"], 0),
        ("KEYSHUFFLE", "Small Key Shuffle",
         ["none", "nearby", "wild", "universal"], 0),
        ("BIGKEYSHUFFLE", "Big Key Shuffle",
         ["none", "nearby", "wild"], 0),
        ("PRIZESHUFFLE", "Prize Shuffle",
         ["none", "dungeon", "nearby", "wild"], 0),
        ("DUNGEON_COUNTERS", "Dungeon Counters",
        ...
        ("LINKED_DROPS", "Linked Drops",
         ["unset", "linked", "independent"], 0),
        ("OVERWORLD_MAP", "Overworld Map",
         ["default", "compass", "map"], 0),
        ("SHUFFLELINKS", "Shuffle Link Houses", ["off", "on"], 0),
        ("SHUFFLETAVERN", "Shuffle Tavern", ["off", "on"], 0),
        ("PSEUDOBOOTS", "Pseudo Boots", ["off", "on"], 0),
...
class Menu:
    ...
    def generate(self):
        ...
        if seed and os.path.isfile(seed):
            ...
            time.sleep(1.0)
            return 0
        error = next((ln[6:].strip() for ln in reversed(out_lines)
                      if ln.startswith("ERROR:")), "")
        self.message(error or "Generation failed. Returning to menu.", RED)
        time.sleep(2.0)
